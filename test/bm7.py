...
# Custom CUDA kernel for velocity and position update
update_kernel = cp.RawKernel(r'''
extern "C" __global__
void update_positions(double* positions, double* velocities, double* forces, double* masses, int num_bodies, double dt) {
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i >= num_bodies) return;
...
class GPUNBodySimulation:
    ...
    def compute_forces(self):
        ...
        # Launch the force computation kernel asynchronously
        force_kernel((grid_size,), (block_size,), 
                     (positions_flat, velocities_flat, self.masses, forces_flat, self.num_bodies, self.cutoff_distance, G),
                     stream=self.stream)
    ...
    def update(self):
        ...
        # Launch the position and velocity update kernel asynchronously
        update_kernel((grid_size,), (block_size,), 
                      (positions_flat, velocities_flat, forces_flat, self.masses, self.num_bodies, self.dt),
                      stream=self.stream)
