...
class GPUNBodySimulation:
    def __init__(self, num_bodies, cutoff_distance, dt=1.0):
        ...
        self.positions = cp.random.rand(num_bodies, 3) * 1000.0  # Random positions in a 1000x1000x1000 box
        self.velocities = cp.zeros((num_bodies, 3))  # Initially at rest
        self.masses = cp.random.rand(num_bodies) * 1e24  # Random masses between 0 and 1e24 kg

    def compute_forces(self):
        # Calculate pairwise differences for all positions
        delta_pos = self.positions[:, cp.newaxis, :] - self.positions[cp.newaxis, :, :]  # Shape (N, N, 3)

        # Calculate pairwise distances (with small epsilon to avoid division by zero)
        dist_sq = cp.sum(delta_pos ** 2, axis=2) + 1e-10  # Shape (N, N)
        dist = cp.sqrt(dist_sq)

        # Avoid self-interaction by setting inverse distances for self-pairs to zero
        inv_dist_cube = cp.where(dist > 1e-10, 1.0 / (dist_sq * dist), 0.0)  # Shape (N, N)

        # Gravitational force calculation: F = G * (m_i * m_j / r^2) * (r_ij / |r_ij|)
        forces = G * (self.masses[:, cp.newaxis] * self.masses[cp.newaxis, :]).reshape(self.num_bodies, self.num_bodies, 1) * inv_dist_cube[:, :, cp.newaxis] * delta_pos
        ...
        repulsive_forces = cp.where(close_bodies[:, :, cp.newaxis], -0.1 * delta_pos, 0.0)
        total_forces += cp.sum(repulsive_forces, axis=1)
    ...
    def update(self):
        ...
        self.velocities += forces / self.masses[:, cp.newaxis] * self.dt
...
bodies_list = [5000, 10000, 20000, 50000]
